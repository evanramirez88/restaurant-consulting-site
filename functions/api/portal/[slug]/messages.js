      ...
      try {
        ...
        if (isValid) {
          ...
          if (client && client.slug === slug) {
            return { authenticated: true, clientId: payload.clientId, clientName: client.name };
          }
        }
  ...
  if (adminToken) {
    ...
    if (jwtSecret) {
      try {
        ...
        if (isValid) {
          const db = env.DB;
          const client = await db.prepare('SELECT id, name FROM clients WHERE slug = ?')
            .bind(slug).first();
          return { authenticated: true, isAdmin: true, clientId: client?.id, clientName: client?.name || 'Admin' };
        }
      } catch (e) { /* fall through */ }
    }
  }
